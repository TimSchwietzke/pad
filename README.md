...
- **Reorder ohne Maus (A11y):** das Zeilen-Menü (⋯) bekommt **„move up" / „move down"** — Aufgaben per Tastatur/Menü umordnen statt nur per Drag; wird als custom-Reihenfolge gespeichert. Bezug ist die **sichtbare** Liste (nicht die rohe Fetch-Order, die auch gefilterte/erledigte Tasks enthält), Optionen an den Rändern korrekt ausgeblendet. Schließt den Critique-A11y-Punkt (Reorder war nur mit Maus möglich)
- **Aufgabe bearbeiten (aufklappbarer Editor):** Klick auf den Aufgaben-Titel klappt eine Inline-Bearbeitung auf — **Titel umbenennen** (leerer Titel wird verworfen) und die bislang ungenutzten **Notizen** bearbeiten (beides optimistisch, speichert bei Verlassen/Enter). Im offenen Zustand sind **alle Param-Chips sichtbar** (schließt den Critique-P3 „Chips nur bei Hover"); die Zeile hebt sich als Panel ab. Schließen per Enter, Escape, Collapse-Pfeil **oder Klick außerhalb der Aufgabe** (Klicks in die portalierten Chip-Menüs zählen nicht). Voll getestet (Umbenennen, leerer-Titel-Schutz, Notiz-Persistenz, Klick-außerhalb)
...
- **Gruppierung als eigene Achse (`group by`):** die Datums-Buckets waren die feste Wirbelsäule der Liste — jetzt sind sie *eine* wählbare Achse neben **projekt**, **priorität** und **none** (flache Liste), als eigenes Dropdown in der Controlbar (geräteweit gespeichert). Gruppieren (welche Unterlisten) × Sortierung (Reihenfolge *innerhalb*) × Dichte sind damit drei saubere Achsen. Projekt-Gruppen tragen ihren Farbpunkt im Header, fertige Aufgaben sinken auf jeder Achse in eine abschließende „done"-Gruppe (außer die Liste zeigt *nur* fertige — dann wird normal gruppiert statt auf einen nutzlosen Haufen zu kollabieren). Umordnen bleibt **innerhalb** einer Gruppe: die Position entscheidet nicht über die Gruppenzugehörigkeit, ein Zug über eine Überschrift hinweg wäre ein unsichtbarer No-op — Drag akzeptiert nur Ziele derselben Gruppe, „move up/down" sind an den Gruppenrändern ausgeblendet. Der Markdown-Export folgt automatisch (H2 je Gruppe). Unit-Tests für die reine Gruppierung + Komponententests (Projekt-/Prioritäts-Achse, gruppenlokales Umordnen)
- **Tastatur-Hilfe („?") + Shortcut-Schicht:** ein Sheet listet, worauf pad ohne Maus hört — erreichbar über `?` oder den neuen `shortcuts`-Eintrag in der Sidebar (Radix-`Dialog`, Fokus-Falle + Escape, Token-Optik in beiden Modi). Dazu eine gemeinsame Shortcut-Schicht: `b` klappt die Sidebar auf/zu, `c` legt an, und **alle** Tastenkürzel schweigen, solange getippt wird oder ein Menü/Dialog offen ist (Radix nutzt dort selbst Buchstaben für Typeahead). Nebenbei zwei Critique-Punkte erledigt: Enter auf **leerem Titel** sagt jetzt „give it a title first" statt still nichts zu tun, und die drei Tastenkappen-Dialekte sind zu einer `.kbd`-Vokabel zusammengezogen; z-Index-Skala um `modal`/`toast` ergänzt (ein Fehler-Toast liegt nie mehr hinter einem Dialog)

- **Wiederholende Aufgaben:** ein `repeat`-Chip an jeder Aufgabe (und schon beim Anlegen) — `daily`, `weekly`, `every 2 weeks`, `monthly`, `yearly` als Presets plus eine freie „every *n* days/weeks/months/years"-Zeile. Die Regel liegt **an der Aufgabe** statt in einer eigenen Template-Tabelle: Abhaken lässt die erledigte Aufgabe als Historie stehen und legt die **nächste Vorkommnis** an — mit allem, was die Aufgabe ausmacht (Projekt, Notizen, Priorität, Aufwand, Tags und die Regel selbst), Deadline um ein Intervall weiter. Damit funktionieren Liste, Sortierung, Gruppierung, Filter und die Undo-Schonfrist unverändert weiter. Details, die zählen: eine **überfällige** Aufgabe holt auf (kommt *nächste* Woche wieder, nicht erneut überfällig), die Monatsarithmetik **klemmt auf die Monatslänge** (31. Jan + 1 Monat → 28./29. Feb statt 3. März), ohne Deadline zählt der Erledigungszeitpunkt, und ein **Undo** in der Schonfrist nimmt die eben erzeugte Vorkommnis zurück — es sei denn, sie wurde schon bearbeitet. Update + Spawn laufen in *einer* Transaktion. Voll getestet (Go-Unit-Tests für die Datumsarithmetik, Integrationstests für Spawn/Undo/Tag-Übernahme, MSW-Komponententests für Chip, Custom-Intervall und den Abhak-Flow)

- **Wochentags-Regeln & Serienende:** eine wöchentliche Wiederholung lässt sich auf **Wochentage** festnageln („jeden Mo + Do") — sieben Umschalter im repeat-Menü, der Chip nennt die Tage statt nur „weekly". Die Tage liegen in einer **eigenen Tabelle** (`todo_recurrence_days`, zusammengesetzter Primärschlüssel) — mehrwertige Attribute bekommen in pad grundsätzlich eine eigene Tabelle statt Bitmaske/CSV/Array, siehe [CLAUDE.md](CLAUDE.md); die Listen-Ansicht bettet sie wie die Tags in *einer* Aggregat-Query ein. Dazu ein **Ende der Serie**: `never` / `on date` / `after n` — das Enddatum stoppt, sobald die nächste Vorkommnis dahinter läge, der Zähler läuft mit jeder Vorkommnis herunter und erlischt bei 0. Die Terminlogik läuft für Wochentags-Regeln nicht mehr über „addiere ein Intervall", sondern sucht den nächsten aktiven Tag in einer Woche, die das Intervall auch abdeckt („alle 2 Wochen Mo + Do" überspringt die Zwischenwoche, statt zu verrutschen). Voll getestet (Go-Unit-Tests für Wochentags-Arithmetik inkl. Wochenwechsel, Intervall-Wochen und Aufholen; Integrationstests für Roundtrip, Vererbung an die nächste Vorkommnis und beide Serienenden; MSW-Komponententests fürs Setzen, Abwählen und Auslaufen)

### WIP / ToDo
- **Gruppierungs-Achsen erweitern** — `none/date/projekt/priorität` stehen; offen bleiben frei definierbare Zeiträume (eigene Buckets statt heute/diese Woche/später) und ggf. eine Tag-Achse. Geplant.
...
- **Wiederholungen ausbauen** — Kadenzen, Wochentage und Serienende stehen; offen bleiben Monats-Regeln wie „jeder erste Montag". Geplant.
