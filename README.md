...
- **Suche in der Topbar (modulübergreifend angelegt):** das Suchfeld oben ist echt — tippen (oder `/`) öffnet darunter ein Ergebnis-Panel, das die Treffer **nach Modul gruppiert** und jede Gruppe benennt (`todo:`; Kalender, Bewerbungen, Mail hängen sich später als weitere Quellen daran). Ein Treffer nennt den Titel und, wenn er über die **Notizen** gefunden wurde, einen kurzen Auszug als Begründung. Auswahl per Klick oder Pfeiltasten + Enter springt in die Liste, **hebt vorher die Filter auf, die die Aufgabe verstecken würden** (sonst zeigt der Sprung auf eine leere Liste), scrollt die Zeile in die Mitte und lässt sie einmal aufblitzen. Serverseitig `?q=` über Titel *und* Notizen (case-insensitive Teilstring; `%`/`_` sind Zeichen, keine Wildcards; Parameter gebunden, kein String-Bau), 200 ms entprellt, Suche nicht persistiert (ein Moment, keine Einstellung). Go-Unit- + Integrationstests (`?q=` durch Router→SQL inkl. Wildcard-Escaping) und vier Komponententests

### WIP / ToDo
- **Weitere Such-Quellen** — das Panel gruppiert schon nach Modul, gefüllt wird bisher nur `todo:`; Projekte/Tags und später Kalender & Mail kommen als eigene Quellen dazu (dann auch die ⌘K-Sprungmarke). Geplant.
